...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Lenovo\\Desktop\\(2.29)金融素养与股票投资决策实验\\experiment 1\\experiment1.py',
...
charge_text = visual.TextStim(win=win, name='charge_text',
    text='',
    font='Open Sans',
...
account_text = visual.TextStim(win=win, name='account_text',
    text='',
    font='Open Sans',
    pos=(-0.45, 0.4), height=0.05, wrapWidth=None, ori=0.0, 
...
assessment_slider = visual.Slider(win=win, name='assessment_slider',
    ...
    labelColor='blue', markerColor='black', lineColor='black', colorSpace='rgb',
    font='Open Sans', labelHeight=0.05,
...
while continueRoutine:
    ...
    if instruction_prac_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'instruction_prac_image.started')
        # update status
    ...
    if instruction_prac_key_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'instruction_prac_key_resp.started')
        # update status
...
for thisPrac_trial in prac_trials:
    ...
    while continueRoutine:
        ...
        # if charge_text is starting this frame...
        if charge_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            charge_text.frameNStart = frameN  # exact frame index
            charge_text.tStart = t  # local t and not account for scr refresh
            charge_text.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(charge_text, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'charge_text.started')
            # update status
            charge_text.status = STARTED
            charge_text.setAutoDraw(True)
        
        # if charge_text is active this frame...
        if charge_text.status == STARTED:
            # update params
            pass
        ...
        if charge_slider.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'charge_slider.started')
            # update status
        ...
        if account_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'account_text.started')
            # update status
        ...
        if charge_button.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'charge_button.started')
            # update status
    ...
    # keep track of which components have finished
    trialComponents = [Balloon, reminder_text, safeaccount_text, investaccount_text, bankButton_key_resp, pumpwarning_text]
    ...
    while continueRoutine:
        ...
        if safeaccount_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'safeaccount_text.started')
            # update status
        ...
        if investaccount_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'investaccount_text.started')
            # update status
        ...
        if bankButton_key_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'bankButton_key_resp.started')
            # update status
        ...
        if pumpwarning_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'pumpwarning_text.started')
            # update status
    ...
    if popped==True:
        if int(probas) == 1:
            feedbackText='气球炸了！你的收益为:' + str(tmp_profit)+'\n请按回车键继续'
            bang.play()
        else:
            stateImage = imageFile
            feedbackText ='气球漏气了！你的收益为:'+ str(tmp_profit)+'\n请按回车键继续'
            leaky.play()
    else:
        feedbackText='你的收益为:'+ str(tmp_profit)+'\n请按回车键继续'
    ...
    continueRoutine = True
    # update component parameters for each repeat
    assessment_text.setText('在决策后，看到决策结果后，\n如果被试感到后悔，请被试评价自己的后悔程度\n（1表示稍微有点后悔，7表示非常后悔）')
    assessment_slider.reset()
    # keep track of which components have finished
    assessmentComponents = [assessment_text, assessment_slider]
    ...
    while continueRoutine:
        ...
        if assessment_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'assessment_text.started')
            # update status
        ...
        if assessment_slider.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'assessment_slider.started')
            # update status
