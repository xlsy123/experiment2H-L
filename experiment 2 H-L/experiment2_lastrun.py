...
expName = 'experiment2'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
    'age': '',
    'gender': ['男','女'],
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Lenovo\\Desktop\\研究二实验材料\\experiment 2 H-L\\experiment2_lastrun.py',
...
formal_intro_image = visual.ImageStim(
    win=win,
    name='formal_intro_image', 
    image='formal_intro_image.jpg', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.1, 0.8),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
formal_intro_key_resp = keyboard.Keyboard()
...
relax_key_resp = keyboard.Keyboard()
relax_image = visual.ImageStim(
    win=win,
    name='relax_image', 
    image='relax.jpg', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.1, 0.8),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-2.0)
clock_movie = visual.MovieStim(
    win, name='clock_movie',
    filename='clock.mp4', movieLib='ffpyplayer',
    loop=False, volume=1.0, noAudio=False,
    pos=(0, -0.35), size=(0.3, 0.18), units=win.units,
...
ending_image = visual.ImageStim(
    win=win,
    name='ending_image', 
    image='thanks.jpg', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.1, 0.8),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
...
for thisPrac_trial in prac_trials:
    ...
    tri_msg = '当前是第' + str(tri_num-3) + '轮，共60轮'
    ...
    while continueRoutine:
        ...
        if earned_key_resp.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # keyboard checking is just starting
            earned_key_resp.clock.reset()  # now t=0
        if earned_key_resp.status == STARTED:
...
continueRoutine = True
# update component parameters for each repeat
formal_intro_key_resp.keys = []
formal_intro_key_resp.rt = []
_formal_intro_key_resp_allKeys = []
# keep track of which components have finished
formal_intro2Components = [formal_intro_image, formal_intro_key_resp]
for thisComponent in formal_intro2Components:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
while continueRoutine:
    ...
    # if formal_intro_image is starting this frame...
    if formal_intro_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        formal_intro_image.frameNStart = frameN  # exact frame index
        formal_intro_image.tStart = t  # local t and not account for scr refresh
        formal_intro_image.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(formal_intro_image, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'formal_intro_image.started')
        # update status
        formal_intro_image.status = STARTED
        formal_intro_image.setAutoDraw(True)
    
    # if formal_intro_image is active this frame...
    if formal_intro_image.status == STARTED:
        # update params
        pass
    ...
    # if formal_intro_key_resp is starting this frame...
    if formal_intro_key_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        formal_intro_key_resp.frameNStart = frameN  # exact frame index
        formal_intro_key_resp.tStart = t  # local t and not account for scr refresh
        formal_intro_key_resp.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(formal_intro_key_resp, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'formal_intro_key_resp.started')
        # update status
        formal_intro_key_resp.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(formal_intro_key_resp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(formal_intro_key_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if formal_intro_key_resp.status == STARTED and not waitOnFlip:
        theseKeys = formal_intro_key_resp.getKeys(keyList=['space'], waitRelease=False)
        _formal_intro_key_resp_allKeys.extend(theseKeys)
        if len(_formal_intro_key_resp_allKeys):
            formal_intro_key_resp.keys = _formal_intro_key_resp_allKeys[-1].name  # just the last key pressed
            formal_intro_key_resp.rt = _formal_intro_key_resp_allKeys[-1].rt
            formal_intro_key_resp.duration = _formal_intro_key_resp_allKeys[-1].duration
            # a response ends the routine
            continueRoutine = False
...
for thisComponent in formal_intro2Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if formal_intro_key_resp.keys in ['', [], None]:  # No response was made
    formal_intro_key_resp.keys = None
thisExp.addData('formal_intro_key_resp.keys',formal_intro_key_resp.keys)
if formal_intro_key_resp.keys != None:  # we had a response
    thisExp.addData('formal_intro_key_resp.rt', formal_intro_key_resp.rt)
    thisExp.addData('formal_intro_key_resp.duration', formal_intro_key_resp.duration)
thisExp.nextEntry()
...
trials = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('exp_condi_H.xlsx'),
...
# keep track of which components have finished
relaxComponents = [relax_key_resp, relax_image, clock_movie]
...
while continueRoutine:
    ...
    # if relax_key_resp is starting this frame...
    if relax_key_resp.status == NOT_STARTED and tThisFlip >= 30-frameTolerance:
        # keep track of start time/frame for later
        relax_key_resp.frameNStart = frameN  # exact frame index
        relax_key_resp.tStart = t  # local t and not account for scr refresh
        relax_key_resp.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(relax_key_resp, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'relax_key_resp.started')
        # update status
        relax_key_resp.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(relax_key_resp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(relax_key_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if relax_key_resp.status == STARTED and not waitOnFlip:
        theseKeys = relax_key_resp.getKeys(keyList=['space'], waitRelease=False)
        _relax_key_resp_allKeys.extend(theseKeys)
        if len(_relax_key_resp_allKeys):
            relax_key_resp.keys = _relax_key_resp_allKeys[-1].name  # just the last key pressed
            relax_key_resp.rt = _relax_key_resp_allKeys[-1].rt
            relax_key_resp.duration = _relax_key_resp_allKeys[-1].duration
            # a response ends the routine
            continueRoutine = False
    
    # *relax_image* updates
    
    # if relax_image is starting this frame...
    if relax_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        relax_image.frameNStart = frameN  # exact frame index
        relax_image.tStart = t  # local t and not account for scr refresh
        relax_image.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(relax_image, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'relax_image.started')
        # update status
        relax_image.status = STARTED
        relax_image.setAutoDraw(True)
    
    # if relax_image is active this frame...
    if relax_image.status == STARTED:
        # update params
        pass
    
    # *clock_movie* updates
    
    # if clock_movie is starting this frame...
    if clock_movie.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        clock_movie.frameNStart = frameN  # exact frame index
        clock_movie.tStart = t  # local t and not account for scr refresh
        clock_movie.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(clock_movie, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'clock_movie.started')
        # update status
        clock_movie.status = STARTED
...
for thisComponent in relaxComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if relax_key_resp.keys in ['', [], None]:  # No response was made
    relax_key_resp.keys = None
thisExp.addData('relax_key_resp.keys',relax_key_resp.keys)
if relax_key_resp.keys != None:  # we had a response
    thisExp.addData('relax_key_resp.rt', relax_key_resp.rt)
    thisExp.addData('relax_key_resp.duration', relax_key_resp.duration)
thisExp.nextEntry()
clock_movie.stop()
...
trials_2 = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('exp_condi_L.xlsx'),
...
for thisTrial_2 in trials_2:
    ...
    for thisComponent in chargeComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_2.addData('charge_slider.response', charge_slider.getRating())
    # the Routine "charge" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    if bankButton_key_resp.keys in ['', [], None]:  # No response was made
        bankButton_key_resp.keys = None
    trials_2.addData('bankButton_key_resp.keys',bankButton_key_resp.keys)
    if bankButton_key_resp.keys != None:  # we had a response
        trials_2.addData('bankButton_key_resp.rt', bankButton_key_resp.rt)
        trials_2.addData('bankButton_key_resp.duration', bankButton_key_resp.duration)
    # the Routine "trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    if feedback_key_resp.keys in ['', [], None]:  # No response was made
        feedback_key_resp.keys = None
    trials_2.addData('feedback_key_resp.keys',feedback_key_resp.keys)
    if feedback_key_resp.keys != None:  # we had a response
        trials_2.addData('feedback_key_resp.rt', feedback_key_resp.rt)
        trials_2.addData('feedback_key_resp.duration', feedback_key_resp.duration)
    # the Routine "feedback" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisComponent in assessmentComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_2.addData('assessment_slider.response', assessment_slider.getRating())
    # the Routine "assessment" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    if earned_key_resp.keys in ['', [], None]:  # No response was made
        earned_key_resp.keys = None
    trials_2.addData('earned_key_resp.keys',earned_key_resp.keys)
    if earned_key_resp.keys != None:  # we had a response
        trials_2.addData('earned_key_resp.rt', earned_key_resp.rt)
        trials_2.addData('earned_key_resp.duration', earned_key_resp.duration)
    # the Routine "earned" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
# keep track of which components have finished
endingComponents = [ending_image, ending_key_resp]
...
while continueRoutine:
    ...
    # if ending_image is starting this frame...
    if ending_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ending_image.frameNStart = frameN  # exact frame index
        ending_image.tStart = t  # local t and not account for scr refresh
        ending_image.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(ending_image, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'ending_image.started')
        # update status
        ending_image.status = STARTED
        ending_image.setAutoDraw(True)
    
    # if ending_image is active this frame...
    if ending_image.status == STARTED:
        # update params
        pass
    ...
    if ending_key_resp.status == NOT_STARTED and t >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ...
        # keyboard checking is just starting
        ending_key_resp.clock.reset()  # now t=0
    if ending_key_resp.status == STARTED:
